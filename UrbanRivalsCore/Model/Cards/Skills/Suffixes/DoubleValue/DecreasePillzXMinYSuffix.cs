...
namespace UrbanRivalsCore.Model.Cards.Skills.Suffixes.DoubleValue
{
    public class DecreasePillzXMinYSuffix : DoubleValueSuffix
    {
        ...
        {
            Regex regex = new Regex(@"^-(?<x>[1-9])(?:OppPillz|PillzOpp)Min(?<y>[0-9])$", RegexOptions.Compiled);

            PRV_PARSER = new DoubleValueSuffixParser(regex, (x, y) => new DecreasePillzXMinYSuffix(x, y), 35);
