...
namespace UrbanRivalsCore.Model.Cards.Skills.Suffixes.DoubleValue
{
    public class DecreaseAttackXPerRemainingPillzMinYSuffix : DoubleValueSuffix
    {
        ...
        {
            Regex regex = new Regex(@"^-(?<x>[1-9])OppAttPerPillzLeftMin(?<y>[1-9][0-9]?)$", RegexOptions.Compiled); 

            PRV_PARSER = new DoubleValueSuffixParser(regex, (x, y) => new DecreaseAttackXPerRemainingPillzMinYSuffix(x, y), 2);
